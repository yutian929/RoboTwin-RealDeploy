...
class RealDP3Deployer:
    ...
    def get_observation(self):
        ...
        pointcloud = self._depth_to_pointcloud(depth_img, color_img, num_points=self.args.pcd_num_points)
    ...
    def run(self, max_steps=300):
        ...
        print("\n" + "="*50)
        print("  DP3 Real Robot Deployment")
        ...
        try:
            while self.running and step < max_steps:
                ...
                self._draw_action_visualization(display_img, actions, current_action_idx=-1, step=step)
                cv2.imshow("DP3 Deployment", display_img)
                
                # If manual mode, wait for SPACE to execute
                if REQUIRE_SPACE_TO_EXECUTE:
                    print(f"Step {step}: Press SPACE to execute {len(actions)} actions...")
                    while True:
                        ...
                        if key == ord(' '):
                            ...
                            self.running = False
                            break
                        # Keep updating display with current camera view
                        obs, color_img = self.get_observation()
                        display_img = color_img.copy()
                        self._draw_action_visualization(display_img, actions, current_action_idx=-1, step=step)
                ...
                for action_idx, action in enumerate(actions):
                    ...
                    obs, color_img = self.get_observation()
                    display_img = color_img.copy()
                    self._draw_action_visualization(display_img, actions, current_action_idx=action_idx, step=step)
                    cv2.imshow("DP3 Deployment", display_img)
...
def main():
    ...
    parser.add_argument('--pcd_num_points', type=int, default=1024, help='Number of points in point cloud (inference only)')
